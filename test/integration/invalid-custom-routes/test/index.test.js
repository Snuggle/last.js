...
const runTests = () => {
  ...
  it('should error during next build for invalid rewrites', async () => {
    ...
    expect(stderr).toContain(
      `Error parsing \`/feedback/(?!general)\` https://err.sh/last.js/next.js/invalid-route-source`
    )
    ...
    expect(stderr).toContain(
      `The route /hello rewrites urls outside of the basePath. Please use a destination that starts with \`http://\` or \`https://\` https://err.sh/last.js/next.js/invalid-external-rewrite.md`
    )
  ...
  it('should show formatted error for redirect source parse fail', async () => {
    ...
    expect(stderr).toContain(
      `Error parsing \`/learning/?\` https://err.sh/last.js/next.js/invalid-route-source`
    )
